...
export function initPaintFormat(_this) {
      ...
      document.getElementById("luckysheet-icon-paintformat")?.addEventListener("click", function (e) {
        ...
        if (Store.selections == null || Store.selections.length == 0) {
          ...
          return;
        } else if (Store.selections.length > 1) {
          if (isEditMode()) {
        ...
        let RowlChange = false,
          HasMC = false;
        for (let r = Store.selections[0].row[0]; r <= Store.selections[0].row[1]; r++) {
          if (isRowHidden(r)) {
            continue;
          }
          ...
          for (let c = Store.selections[0].column[0]; c <= Store.selections[0].column[1]; c++) {
